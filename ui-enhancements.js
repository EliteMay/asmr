...
(function(){
  ...
  function showDashboard(){try{window.asmrtubeBrowse?.hide?.()}catch{}renderDashboard();document.body.classList.add('dashboard-mode');$('.view-btn').forEach(button=>button.classList.toggle('active',button.id==='dashboardBtn'));$('#viewEyebrow').textContent='LIBRARY OVERVIEW';$('#nowTitle').textContent='ライブラリ概要';$('#nowCreator').textContent='登録状況と最近聴いたASMR';closeMobileSidebar();diag('dashboard.open')}
  ...
  function initMobileNavigation(){
    ...
    sidebar.addEventListener('click',event=>{if(matchMedia('(max-width:900px)').matches&&event.target.closest('.song-item,.view-btn,.playlist-item,.channel-item,.sidebar-page-btn'))setTimeout(closeMobileSidebar,30)});
  }
